...
class RatValFunc(Mapping):
    ...
    >>> f = RatValFunc({'a': 1.1, 'b': '-1/2','c': 0})
    >>> f
    RatValFunc({'a': Fraction(11, 10), 'c': Fraction(0, 1), 'b': Fraction(-1, 2)})
    >>> f['a']
    Fraction(11, 10)
    >>> g = RatValFunc({'b': '.6', 'c': -2, 'd': 0.0})
    >>> g
    RatValFunc({'c': Fraction(-2, 1), 'b': Fraction(3, 5), 'd': Fraction(0, 1)})
    >>> (.3 * f - g) / 2
    RatValFunc({'c': Fraction(1, 1), 'b': Fraction(-3, 8)})
